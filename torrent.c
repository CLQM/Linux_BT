...
int download_upload_with_peers()
{
	...
	for(;;) 
	{
		...
		if(now_time-last_time[1] >= 30) 
		{
			if(download_piece_num > 0 && peer_head != NULL) 
			{
				select_optunchoke_peer();//选择优化非阻塞Peer
				last_time[1] = now_time;//更新时间
			}
		}
			...
			connecting_tracker != 1 && connect_peer != 1 && connecting_peer != 1) 
		{
			...
			connect_tracker = 0;//不需要连接Tracker
			connecting_tracker = 1;//已经与Tracker建立连接
			start_connect_tracker = now_time;//开始连接tracker的时间
		}
		...
		if(connect_peer == 1) 
		{
			...
			connect_peer = 0;
			connecting_peer = 1;
			start_connect_peer = now_time;//开始连接Peer的时间
		}

		FD_ZERO(&rset);//清空读文件描述符集合
		FD_ZERO(&wset);//清空写文件描述符集合
		...
		if(connecting_tracker == 1) 
		{
			...
			if(now_time-start_connect_tracker > 10) 
			{
				for(i = 0; i < tracker_count; i++)
					if(valid[i] != 0)//valid[i]==1表明已经建立连接  
						close(sock[i]);//关闭连接
			} 
			else 
			{
